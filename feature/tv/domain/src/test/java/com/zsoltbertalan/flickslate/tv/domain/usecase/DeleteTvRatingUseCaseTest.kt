...
class DeleteTvRatingUseCaseTest {
	private val getSessionIdUseCase: GetSessionIdUseCase = mockk()
	private val tvRatingsRepository: TvRatingsRepository = mockk()
	private val useCase = DeleteTvRatingUseCase(getSessionIdUseCase, tvRatingsRepository)

	@Test
	fun `execute returns success when dependencies succeed`() = runTest {
		coEvery { getSessionIdUseCase.execute() } returns Ok("session")
		coEvery { tvRatingsRepository.deleteTvShowRating(tvShowId = 42, sessionId = "session") } returns Ok(Unit)

		useCase.execute(tvShowId = 42) shouldBeEqual Ok(Unit)
	}
	...
	fun `execute returns failure when session retrieval fails`() = runTest {
		...
		useCase.execute(tvShowId = 42) shouldBeEqual Err(failure)
	}
	...
	fun `execute returns failure when repository fails`() = runTest {
		...
		coEvery { getSessionIdUseCase.execute() } returns Ok("session")
		coEvery { tvRatingsRepository.deleteTvShowRating(tvShowId = 42, sessionId = "session") } returns Err(failure)
