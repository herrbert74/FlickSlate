...
class RateTvShowEpisodeUseCaseTest {

	private val getSessionIdUseCase: GetSessionIdUseCase = mockk()
	private val tvRatingsRepository: TvRatingsRepository = mockk()
	private val useCase = RateTvShowEpisodeUseCase(getSessionIdUseCase, tvRatingsRepository)
	...
	fun `execute returns success when session and rating succeed`() = runTest {
		...
		coEvery { tvRatingsRepository.rateTvShowEpisode(any(), any(), any(), any(), any()) } returns Ok(Unit)

		useCase.execute(tvShowId = 1, seasonNumber = 2, episodeNumber = 3, rating = 7.5f) shouldBeEqual Ok(Unit)
	}
	...
	fun `execute returns failure when session retrieval fails`() = runTest {
		...
		useCase.execute(tvShowId = 1, seasonNumber = 2, episodeNumber = 3, rating = 7.5f) shouldBeEqual Err(failure)
	}
	...
	fun `execute returns failure when repository fails`() = runTest {
		...
		coEvery { getSessionIdUseCase.execute() } returns Ok("session")
		coEvery { tvRatingsRepository.rateTvShowEpisode(any(), any(), any(), any(), any()) } returns Err(failure)
