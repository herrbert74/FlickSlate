...
class PopularMoviesRemoteDataSourceTest {
	...
	@Before
	fun setup() {
		coEvery { moviesService.getPopularMovies(any(), any(), any()) } returns Response.success(
			MovieDtoMother.createPopularMovieList()
		)
		popularMoviesRemoteDataSource = PopularMoviesRemoteDataSource(moviesService)
	}

	@Test
	fun `when getPopularMovies called and service returns result then returns correct result`() = runTest {
		val result = popularMoviesRemoteDataSource.getPopularMovies("", 1)
		result.value.pagingList shouldBeEqual MovieMother.createPopularMovieList()
	...
	fun `when getPopularMovies called and service returns failure then returns correct result`() = runTest {
		coEvery { moviesService.getPopularMovies(any(), any(), any()) } returns failNetworkRequestResponse()()
		popularMoviesRemoteDataSource.getPopularMovies("", 1) shouldBeEqual
			Err(
				Failure.ServerError(
					"""
						{"success":false,
	...
	@Suppress("unused")
	private fun failNetworkRequestResponse(): () -> Response<MoviesReplyDto> = {
